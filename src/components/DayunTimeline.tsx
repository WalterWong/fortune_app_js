...
export default function DayunTimeline({ dayun, currentAge }: DayunTimelineProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-xl font-bold mb-4">大運 (十年運)</h2>
      ...
      <div className="grid grid-cols-5 md:grid-cols-10 gap-2">
        {dayun.slice(0, 10).map((cycle, index) => {
          ...
          return (
            ...
            >
              <div className="text-lg font-bold">{cycle.pillar}</div>
              <div className="text-xs text-gray-600">{cycle.ageRange}歲</div>
              {isCurrent && (
                <div className="text-xs text-blue-600 font-medium mt-1">當前</div>
              )}
            </div>
          );
        })}
